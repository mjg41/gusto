from gusto import *
from firedrake import SpatialCoordinate, sin, pi, cos, exp, ln, Function, solve, TestFunction, TrialFunction, dx, File
import sys
...
columns = (int(L/delta_x), int(W/delta_x))  # number of columns
f0 = 1.e-4
beta = 2.e-11
domain = ChannelDomain(L, W, H, columns, nlayers, coriolis=(f0, beta))
...
def Phi_prime(eta):
    return 0.5*u0*((f0 - beta*y0)*(y - 0.5*W*(1 + sin(2*pi*y/W)/pi)) + 0.5*beta*(y**2 - W*(y*sin(2*pi*y/W)/pi + 0.5*W/(pi**2)*cos(2*pi*y/W) - W/3. - 0.5*W/(pi**2))))
def Phi(eta):
...
rho0.assign(rho_b)
u = -u0*sin(pi*y/W)**2*ln(eta)*eta**(-ln(eta)/b**2)
