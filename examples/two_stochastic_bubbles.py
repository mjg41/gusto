...
output = OutputParameters(dirname='two_stoch_bubbles_wide', dumpfreq=10, dumplist=['u', 'p', 'b'], perturbation_fields=[], log_level='INFO')
...
b_pert1 = Function(Vb).interpolate(conditional(r > rc, 0.0,
...
bdash = -bdash
r = sqrt((x - xc) ** 2 + (z - zc) ** 2)
b_pert2 = Function(Vb).interpolate(conditional(r > rc, 0.0,
                                               bdash * (cos(pi * r / (rc * 2.0))) ** 2))

# define initial buoyancy
b0.assign(b_b + b_pert1 + b_pert2)
p0.assign(p_b)
