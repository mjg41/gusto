...
mesh = RectangleMesh(columns, nlayers, L, H)
...
bc_val = hm*exp(-((x-xc)/a)**2)*(cos(pi*(x-xc)/4000))**2
bcs = [DirichletBC(V, Constant(0.), "top"),
              DirichletBC(V, Constant(0.), "bottom")]
soln = Function(V, name="soln")
prob = LinearVariationalProblem(a, L, soln, bcs=bcs)
solver = LinearVariationalSolver(prob)
solver.solve()
asoln = Function(V, name="asoln").interpolate((w0/sinh(pi))*sin(pi*x)*sinh(pi*y))
